 ...
 */
export const watermarkPlacements = schema.table("watermark_placements", {
    id: uuid("id").defaultRandom().primaryKey(),
    name: varchar("name", { length: 120 }).notNull(),
    imageAssetId: uuid("image_asset_id").references(() => watermarkImageAssets.id, {
        onDelete: "set null",
        onUpdate: "restrict",
    }),
...
export const watermarkImageAssetsRelations = relations(watermarkImageAssets, ({ one, many }) => ({
...
export const watermarkPlacementsRelations = relations(watermarkPlacements, ({ one }) => ({
