/**
 * Repair: apply 0049_rare_madrox.sql when drizzle migrator skipped pending SQL
 * (applied row count >= journal length) but tables/config were never created.
 */
...
try {
    const [exists] = await sql<{ exists: boolean }[]>`
        SELECT EXISTS (
            ...
            WHERE table_schema = 'sohoa_app'
              AND table_name = 'disposal_review_council_item_evaluations'
        ) AS exists
    `;

    if (exists?.exists) {
        console.log("✅ disposal_review_council_item_evaluations already exists");
    } else {
        ...
        console.log("✅ Council item evaluations schema applied");
    }
    ...
    console.error("❌ Failed to apply 0049 repair", error);
    Deno.exit(1);
} finally {
