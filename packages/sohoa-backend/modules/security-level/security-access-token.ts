...
export async function verifySecurityAccessToken(input: {
    ...
    try {
        ...
        return payload.sub === input.userId
...
export async function verifyLevelPassword(input: {
    ...
    const [level] = await db
        .select()
        .from(securityLevels)
        .where(and(eq(securityLevels.id, input.securityLevelId), isNull(securityLevels.deletedAt)))
        .limit(1);
...
export async function assertPasswordGates(input: {
    ...
    const levelId = input.resourceSecurityLevelId
        ?? (await getLowestActiveLevel())?.id;
    if (!levelId) return;

    const requirePassword = await getEffectiveBool(levelId, FlagRuleKey.requirePassword);
    if (requirePassword) {
        const [level] = await db
        ...
        if (level?.passwordHash) {
            const ok = await verifySecurityAccessToken({
                token: input.levelToken,
                userId: input.userId,
                scope: "level",
                resourceId: levelId,
            });
            if (!ok) {
                throw httpError.forbidden("PASSWORD_REQUIRED:level");
            }
        }
    }
