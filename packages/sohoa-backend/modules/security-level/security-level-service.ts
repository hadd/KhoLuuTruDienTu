...
async function assertLevelOrderAvailable(
  levelOrder: number,
  excludeId?: string,
) {
...
export const SecurityLevelService = {
  ...
  async list(urlQuery: Parameters<typeof crud.list>[0]) {
    ...
    return {
      ...result,
      items: (result.items as Array<typeof securityLevels.$inferSelect>).map(
        toPublicRecord,
      ),
    };
  },
  ...
  async update(id: string, input: UpdateSecurityLevelInput) {
    ...
    if (
      input.levelOrder !== undefined &&
      input.levelOrder !== existing.levelOrder
    ) {
  ...
  async delete(id: string) {
    ...
    const [usage] = await db
      .select({ total: count() })
      .from(dossiers)
      .where(and(eq(dossiers.securityLevelId, id), isNull(dossiers.deletedAt)));
  ...
  async patchRules(
    ...
    const encryptPinPatch = input.rules.find(
      (r) => r.ruleKey === PermissionRuleKey.encryptDownload,
    );
    const encryptDossierPatch = input.rules.find(
      (r) => r.ruleKey === PermissionRuleKey.encryptDownloadDossier,
    );
    let encryptPinEffective = Boolean(
      currentEffective.find(
        (r) => r.ruleKey === PermissionRuleKey.encryptDownload,
      )?.effectiveValue,
    );
    let encryptDossierEffective = Boolean(
      currentEffective.find(
        (r) => r.ruleKey === PermissionRuleKey.encryptDownloadDossier,
      )?.effectiveValue,
    );
    if (encryptPinPatch) {
      encryptPinEffective = Boolean(encryptPinPatch.value);
    }
    if (encryptDossierPatch) {
      encryptDossierEffective = Boolean(encryptDossierPatch.value);
    }
    if (encryptPinEffective && encryptDossierEffective) {
      throw httpError.badRequest(
        "Chỉ được bật một trong hai: mã hóa ZIP bằng PIN cá nhân hoặc bằng mật khẩu hồ sơ.",
      );
    }

    const downloadPatch = input.rules.find(
      (r) => r.ruleKey === PermissionRuleKey.download,
    );
    const watermarkPatch = input.rules.find(
      (r) => r.ruleKey === PermissionRuleKey.downloadWatermark,
    );
    let downloadEffective = Boolean(
      currentEffective.find((r) => r.ruleKey === PermissionRuleKey.download)
        ?.effectiveValue,
    );
    let watermarkEffective = Boolean(
      currentEffective.find(
        (r) => r.ruleKey === PermissionRuleKey.downloadWatermark,
      )?.effectiveValue,
    );
    if (downloadPatch) {
      downloadEffective = Boolean(downloadPatch.value);
    }
    if (watermarkPatch) {
      watermarkEffective = Boolean(watermarkPatch.value);
    }
    if (watermarkEffective && !downloadEffective) {
      throw httpError.badRequest(
        "Phải bật Tải tài liệu trước khi bật Watermark.",
      );
    }
    if (
      !downloadEffective &&
      (encryptPinEffective || encryptDossierEffective)
    ) {
      throw httpError.badRequest(
        "Phải bật Tải tài liệu trước khi bật mã hóa ZIP.",
      );
    }
