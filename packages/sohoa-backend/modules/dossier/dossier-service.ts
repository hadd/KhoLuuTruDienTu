...
import {
  buildFolderMetadataExportZipStreamIncremental,
...
async function assignDossierToUser(input: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    await cancelInProgressAssignmentsForReassign(tx, {
      ...
      now,
      roles:
        input.role === WorkerRole.MAKER ? [WorkerRole.MAKER] : [input.role],
    });
...
export async function findDossiersInLeafFoldersWithFiles(folderId: string) {
  ...
  const folderById = new Map(
    subtreeFolders.map((folder) => [folder.id, folder]),
  );
  const folderIds = subtreeFolders.map((folder) => folder.id);
  ...
  if (folderIds.length === 0) {
    return {
      rootFolder,
      leafFolders: [],
      dossiers: [] as DossierAssignTarget[],
    };
  }
...
async function validateApprovedFoldersMetadataExport(
  ...
  for (const folderId of uniqueIds) {
    const { rootFolder, dossiers: subtreeDossiers } =
      await findDossiersInFolderSubtree(folderId);
    rootFolders.push(rootFolder);
    for (const dossier of subtreeDossiers) {
      dossierById.set(dossier.id, dossier);
    }
  }
  ...
  if (allDossiers.length === 0) {
    throw httpError.badRequest("No dossiers found in the selected folders");
  }

  if (!options?.bypassStatus) {
    const notApproved = allDossiers.filter(
...
async function loadDossiersForMetadataExport(
  ...
  const rows = await db.query.dossiers.findMany({
    where: activeDossierWhere(inArray(dossiers.id, uniqueIds)),
    with: { files: true },
  });
...
async function buildApprovedMetadataExportZip(
  ...
  input: MetadataExportInput | undefined,
  options: {
    zipBaseName: string;
...
async function assignDossiersByFolderId(input: {
  ...
  const [dossierRecords, activeAssignments, completedMakers] =
    await Promise.all([
  ...
  const dossierById = new Map(
    dossierRecords.map((dossier) => [dossier.id, dossier]),
  );
  const activeMakerIndex =
    input.role === WorkerRole.MAKER
...
export async function resolveGroupAssignFolderId(
  ...
  for (const ancestor of ancestors) {
    const { dossiers: targets } = await findDossiersInLeafFoldersWithFiles(
      ancestor.id,
    );
...
async function listMyDraftAssignments(assigneeId: string) {
  const draftListBlockedDossierStatuses = new Set<string>([
    ...
    ...QC_CHECKER_WORKFLOW.flatMap((config) => [
      config.waiting,
      config.processing,
    ]),
  ]);
...
async function loadFolderSubtreeForBulkDelete(
  folderId: string,
  permanent: boolean,
) {
  const rootFolder = await db.query.folders.findFirst({
...
export const DossierService = {
  ...
  async saveDossierMetadata(
    ...
    if (allowedFields !== null) {
      ...
      const { allowed, violations } = validateWritePermission(
        metadata,
        allowedFields,
      );
      if (!allowed) {
    ...
    const partialBase = ocrBase.replace(
      /(^|\/)metadata\//,
      "$1metadata_partial/",
    );
    ...
    const result = await db.transaction(async (tx) => {
      const [assignmentRow] = await tx
        ...
        .set({
          ...
          workQuality:
            assignment.workQuality === WorkQuality.INCORRECT
      ...
      const hasFieldLevelAcl =
        allowedFields !== null ||
        completedMakers.some(
          (m) => parseAllowedFields(m.allowedFields) !== null,
        );
      ...
      const skipQc = dossier.requiredQcCount === 0;
      const { hasBlockingIssueReportsForDossier, IssueReportService } =
        await import("../issue-report/issue-report-service.ts");
      const hasBlockingIssue = await hasBlockingIssueReportsForDossier(
        dossierId,
        tx,
      );
    ...
    if (
      !result.partial &&
      result.dossierStatus === DossierStatus.WAITING_CHECKER_1
    ) {
      scheduleEditorsCompletedNotification({
        dossierId,
        dossierName: dossier.name,
  ...
  async getDossierMetadataDraft(dossierId: string, actorId: string) {
    ...
    if (!draftKey) {
      throw httpError.badRequest(
        "Cannot resolve draft metadata key for dossier",
      );
    }

    const metadata = await downloadJsonFromStorage(
      resolveMetadataJsonKey(draftKey),
    );
  ...
  async exportMetadataExcelByIds(
    dossierIds: string[],
  ...
  async getDossierMetadataExportFields(dossierId: string) {
    ...
    if (!dossier.currentMetadataKey) {
      throw httpError.badRequest(
        `Dossier "${dossier.name}" has no current metadata`,
      );
    }
  ...
  async previewApprovedMetadataExportByFolder(
    ...
    return await this.previewApprovedMetadataExportByFolders([folderId], input);
  },

  async previewApprovedMetadataExportByFolders(
    folderIds: string[],
    input: {
      presetId?: string;
      columns?: MetadataExportConfig["columns"];
  ...
  async listApprovedExportDossierIds(
    folderIds: string[],
    options?: { bypassStatus?: boolean },
  ): Promise<string[]> {
    const { dossiers: allDossiers } =
  ...
  async getFolderMetadataExportFields(
    folderId: string,
    options?: { bypassStatus?: boolean },
  ) {
    const { dossiers: allDossiers } =
      await validateApprovedFolderMetadataExport(folderId, options);
