...
async function resolveProjectManagerId(
  ...
  if (!projectCode) {
    throw httpError.badRequest(
      "Hồ sơ chưa gắn dự án — không thể chuyển tiếp quản lý dự án",
    );
  }
...
async function resumeDossierForMakerRework(
  ...
  if (
    !DOSSIER_STATUSES_RESUME_MAKER_REWORK.includes(
...
export const IssueReportService = {
  ...
  async listOpenForDossiers(
    ...
    for (const row of rows) {
      ...
      list.push(toResponse(row, row.reporter?.fullName ?? null));
  ...
  async escalate(reportId: string, actorId: string) {
    ...
    if (row.status !== IssueReportStatus.PENDING) {
      throw httpError.conflict(
        "Chỉ có thể chuyển tiếp thông báo đang chờ xử lý",
      );
    }
  ...
  async closeByProjectManager(
    ...
    if (
      ...
      row.escalatedToId !== actorId
    ) {
      throw httpError.forbidden(
        "Bạn không phải quản lý dự án được chuyển tiếp",
      );
    }
    ...
    const result = await db.transaction(async (tx) => {
      ...
      let dossierStatusAfterClose: DossierStatusType | null =
        dossier?.status ?? null;
      ...
      if (dossier?.status === DossierStatus.WAITING_ISSUE_RESOLUTION) {
        ...
        if (!hasOtherEscalated) {
          ...
          if (errorRow) {
            dossierStatusAfterClose = DossierStatus.ERROR;
            await tx.insert(workflowLogs).values({
              dossierId: dossier.id,
              actorId,
              action: "MARK_ERROR_AFTER_ISSUE_CLOSED",
              fromStatus: DossierStatus.WAITING_ISSUE_RESOLUTION,
              toStatus: DossierStatus.ERROR,
  ...
  async listForProjectManager(input: {
    managerId: string;
    /** Admin (global scope): xem mọi issue escalate. PM: chỉ issue chuyển tới mình. */
    ...
    const conditions: Array<
      ReturnType<typeof eq> | ReturnType<typeof inArray>
    > = [];

    if (input.scopeType !== "global") {
      conditions.push(eq(dossierIssueReports.escalatedToId, input.managerId));
    }
  ...
  async closeConfirmedForResetMakers(
    ...
    await tx
      ...
      .where(
        and(
          inArray(
            dossierIssueReports.reporterAssignmentId,
            resetMakerAssignmentIds,
          ),
          eq(dossierIssueReports.status, IssueReportStatus.CONFIRMED),
        ),
      );
