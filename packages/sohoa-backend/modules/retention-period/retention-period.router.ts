...
export function createRetentionPeriodRouter(basePath: string = "/retention-periods") {
    const meta = service.getMetadata?.();
    const tags = [["RetentionPeriod", ...(meta?.tags || [])].join(" ")];
    const docs = service.getDocs({ tags });

    const app = new Elysia({
        name: "retentionPeriodRouter",
        prefix: basePath,
    })
    ...
    app.post(
        ...
        async ({ body, profile, set }) => {
            authHelper.checkPermission(profile, Permission.RETENTION_PERIODS_CREATE);
    ...
    app.put(
        ...
        async ({ params, body, profile }) => {
            authHelper.checkPermission(profile, Permission.RETENTION_PERIODS_UPDATE);
    ...
    app.delete(
        "/:id",
        async ({ params, profile }) => {
            authHelper.checkPermission(profile, Permission.RETENTION_PERIODS_DELETE);
