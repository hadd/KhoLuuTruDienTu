...
export function isPermissionLocked(
  ...
  return getDefaultPermissionsForRole(roleId).includes(permissionKey)
}
...
export function canRevokePermission(
  ...
  return !isPermissionLocked(roleId, permissionKey)
}

export function isModuleRevokeDisabled(
  roleId: string,
  permissions: Array<string>,
  module: string,
  moduleKeys: Array<string>,
  ...
  return grantedKeys.every((key) => isPermissionLocked(roleId, key))
}

export function ensureLockedPermissionsIncluded(
  roleId: string,
  permissions: Array<string>,
): Array<string> {
  if (!isLockedRole(roleId)) {
  ...
  const lockedEntries = getDefaultPermissionsForRole(roleId)
  if (lockedEntries.includes(FULL_ACCESS_PERMISSION)) {
...
export function setPermissionGrantedRespectingLocks(
  roleId: string,
  permissions: Array<string>,
  permissionKey: string,
  module: string,
  ...
  if (!granted && !canRevokePermission(roleId, permissionKey)) {
    return permissions
  }

  const next = setPermissionGranted(
    permissions,
    permissionKey,
    module,
    moduleKeys,
  ...
  return ensureLockedPermissionsIncluded(roleId, next)
}

export function setModuleGrantedRespectingLocks(
  roleId: string,
  permissions: Array<string>,
  module: string,
  moduleKeys: Array<string>,
