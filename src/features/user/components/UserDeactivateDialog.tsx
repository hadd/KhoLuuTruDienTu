...
export function UserDeactivateDialog({
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        t('actions.statusChangeSuccess', 'Status updated successfully'),
      )
        ...
        <AlertDialogHeader>
          <AlertDialogTitle>
            {isActive
              ...
              : t('dialog.activateTitle', 'Mở khóa tài khoản?')}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {isActive
              ? t('dialog.deactivateDescription')
              : t(
                  'dialog.activateDescription',
                  'Bạn chắc chắn muốn mở khóa tài khoản này? Người dùng sẽ có thể đăng nhập bình thường.',
                )}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={handleConfirm} disabled={isPending}>
            {isActive
              ? t('actions.deactivateConfirm', 'Khóa tài khoản')
              : t('actions.activateConfirm', 'Xác nhận mở khóa')}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
