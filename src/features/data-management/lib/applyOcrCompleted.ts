...
export async function applyOcrCompleted(options: {
  ...
  const treeAfterStatusUpdate = queryClient.getQueryData<DataTreeNodeT>(
    dataManagementTreeQueryKey(role),
  )
  if (treeAfterStatusUpdate) {
    const folderIds = resolveOcrReloadFolderIds(treeAfterStatusUpdate, payload)
    for (const folderId of folderIds) {
      try {
        clearLoadedNodeCache(folderId)
        const tree = await loadChildren(folderId)
