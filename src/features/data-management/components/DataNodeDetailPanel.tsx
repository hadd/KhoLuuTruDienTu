...
export function DataNodeDetailPanel({ node }: { node: DataTreeNodeT | null }) {
    ...
    return (
      <Card variant="detail" className="flex min-h-0 flex-1 flex-col">
        <CardContent className="flex flex-1 items-center justify-center py-12">
          <p className="text-center text-sm text-muted-foreground">{t('detail.emptySelection')}</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card variant="detail" className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
        <dl className="grid shrink-0 gap-2 text-sm sm:grid-cols-2">
          <div>
            ...
            <dd className="font-medium text-foreground">{t(`nodeType.${node.type}` as const)}</dd>
          </div>
          <div>
            <dt className="text-muted-foreground">{t('detail.size')}</dt>
            <dd className="font-medium text-foreground">{formatFileSize(node.sizeBytes)}</dd>
          </div>
          <div>
          ...
          <>
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-2">
              ...
              <div className="min-h-0 flex-1 overflow-hidden">
                <PdfViewer fileUrl={node.fileUrl} fileName={node.name} className="h-full min-h-[320px]" />
              </div>
            </div>
          </>
        ) : null}
