...
export const Route = createFileRoute('/app/data-config/watermark-configs')({
  beforeLoad: async ({ context }) => {
    await requirePermission(
      context,
      APP_SCREEN_ACCESS.dataConfig.watermarkConfigs,
    )
  },
  validateSearch: (raw) => watermarkConfigSearchSchema.parse(raw),
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(watermarkPlacementsQueryOptions())
    return {}
  },
  staticData: {
    crumb: () => ({
      label: i18n.t('tiles.watermarkConfigs', { ns: 'data-config' }),
      parent: {
