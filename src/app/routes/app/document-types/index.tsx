...
export const Route = createFileRoute('/app/document-types/')({
  staticData: {
    crumb: () => i18n.t('admin.generalCatalog.documentTypeCatalog', { ns: 'common' }),
  },
  ...
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(
      documentTypesQueryOptions({ page: 1, limit: DEFAULT_LIST_PAGE_LIMIT }),
    )
    return {}
  },
  head: () => ({
    meta: [
      {
        title: `${i18n.t('title', { ns: 'document-type' })} - ${i18n.t('appName', { ns: 'common' })}`,
      },
    ],
  }),
  component: DocumentTypeRoute,
  errorComponent: DocumentTypeErrorComponent,
})

function DocumentTypeRoute() {
  return <DocumentTypeManagementPage />
}

function DocumentTypeErrorComponent({
  error,
  reset,
}: {
  error: unknown
