...
import {
  ...
  isDigitizationHubVisible,
  loadPermissionContext,
  resolvePermissionFallbackPath,
} from '@/features/auth/lib/permission-access'
import { requireAuth } from '@/features/auth/routeGuards'
import { DigitizationHubPage } from '@/features/navigation/components/DigitizationHubPage'
import { permissionsCatalogQueryOptions } from '@/features/permissions/queries'
import i18n from '@/lib/i18n/config'

export const Route = createFileRoute('/app/digitization-hub/')({
  staticData: {
    crumb: () => i18n.t('admin.groups.digitization', { ns: 'common' }),
  },
  beforeLoad: async ({ context }) => {
    ...
    if (!isDigitizationHubVisible(permissions, catalog)) {
      throw redirect({
  ...
  head: () => ({
    meta: [
      {
        title: `${i18n.t('admin.groups.digitization', { ns: 'common' })} - ${i18n.t('appName', { ns: 'common' })}`,
      },
    ],
  }),
  component: DigitizationHubPage,
})
