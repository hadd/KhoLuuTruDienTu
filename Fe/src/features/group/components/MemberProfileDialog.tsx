    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription>Chi tiết thông tin cá nhân của người dùng.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4 max-h-[80vh] overflow-y-auto pr-2">
          <div className="flex flex-col items-center justify-center space-y-2 mb-6">
            ...
            <span className="text-sm text-muted-foreground bg-muted px-2 py-1 rounded-full">
              {member.role === 'leader' ? 'Leader' : member.role === 'manager' ? 'Người duyệt' : 'Thành viên'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-sm border-t pt-4">
            ...
            <div className="font-semibold text-muted-foreground">Ngày tham gia:</div>
          ...
          <div className="border-t pt-4 mt-4">
            ...
            <div className="space-y-3">
              {(member.documents || []).map((task) => (
                <div key={task.id} className="flex flex-col gap-2 border p-3 rounded-md bg-muted/10 hover:bg-muted/30 transition-colors">
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex flex-col">
                      <span className="font-medium text-sm text-foreground">{task.id}</span>
