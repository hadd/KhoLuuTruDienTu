...
describe('metadataLayout', () => {
  it('partitions TT05 groups into ho so and tai lieu sections', () => {
    const partition = partitionMetadataGroupsForDisplay(tt05Groups)

    expect(partition.layout).toBe('tt05')
    expect(partition.hoSoEntry?.groupIndex).toBe(0)
  ...
  it('uses TEN_LOAI_TAI_LIEU as document card title', () => {
    expect(getTaiLieuDocumentDisplayTitle(tt05Groups[1]!)).toBe('Quyet dinh')
  })
